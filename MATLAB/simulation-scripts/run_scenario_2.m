...
scmatPath = strcat(pathBase,'/data/sc_matrices_used_scenario_2/'); 
% Add path to software 
addpath(strcat(pathBase,'/MATLAB/software'));
% Add path to util functions
addpath(strcat(pathBase,'/MATLAB/simulation-scripts/utils'));
% Define path to save results 
objsavepath = strcat(pathBase, '/results/results_scenario_2.txt');
% Define path to save simulation status  
statusFilePath = strcat(pathBase, '/results/status_file_scenario_2.txt');


%% PREPARE CONNECTIVITY MATRICES BASED ON PRECOMPUTED "BASE" MATRICES

% File names of pre-computed connectivity matrices 
scMatrixNameGrid     = {'A1a.txt', 'A1b.txt', 'A1c.txt', 'A1d.txt',...
                        'A2a.txt', 'A2b.txt', 'A2c.txt', 'A2d.txt',...
                        'A3a.txt', 'A3b.txt', 'A3c.txt', 'A3d.txt',...
                        'A4a.txt', 'A4b.txt', 'A4c.txt', 'A4d.txt'};
% Values of dissimilarity between "base" co nectivity 
% matrices and matrices resulted in turning some of the "base" entries from
% positive to negative
scMatrixDiffMaxVals  = [1.0, 1.0, 1.0, 1.0,...
                        0.96, 0.96, 0.96, 0.96,...
                        0.68, 0.68, 0.68, 0.68,...
                        0.85, 0.85, 0.85, 0.85]; 
...
scMatrixAobsList = {}; 
% Array to store values of dissimilarity
scMatrixDiffGridList = {};

% Values for local testing: % iScMatrixIdx = 3; iScMatrixDiffIdx = 3;
% Iterate over 16 matrices 
for iScMatrixIdx = 1:length(scMatrixNameGrid)   
	fprintf('iScMatrixIdx: %d\n', iScMatrixIdx);
    ...
    A                  = importdata(strcat(scmatPath, iScMatrixName));   
    % Iterate over matrix dissimilarity parameters 
    for iScMatrixDiffIdx = 1:length(iScMatrixDiffGrid)   
        fprintf('iScMatrixDiffIdx: %d\n', iScMatrixDiffIdx);
        iScMatrixDiff  = iScMatrixDiffGrid(iScMatrixDiffIdx);
        [Aobs,~,~,~]   = rand_xswap_wu_wrapper_fix_diss(A,iScMatrixDiff,1000,1,50000); 
        scMatrixAobsList_TMP{iScMatrixDiffIdx} = Aobs; 
    end 
    fprintf('length(scMatrixAobsList_TMP): %d\n', length(scMatrixAobsList_TMP));
...
length(scMatrixAobsList) == 16
length(scMatrixDiffGridList) == 16
sum(cellfun('length', scMatrixAobsList)) == 128
sum(cellfun('length', scMatrixDiffGridList)) == 128
...
for iScMatrixIdx = 1:length(scMatrixAobsList)  
    ...
    for in = nGrid 
        ...
        for iScMatrixDiffIdx = 1:length(iScMatrixAobsGrid)     
            ...
            iScMatrixDiff  = iScMatrixDiffGrid(iScMatrixDiffIdx); 
            Aobs           = abs(iScMatrixAobsGrid{iScMatrixDiffIdx}); 
